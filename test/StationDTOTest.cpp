...
static void SerializeLineTest()
{
    ...
    station_guingamp->linked_lines = oatpp::List<oatpp::String>::createShared(); 
    for(auto line : { "LER_BRE0", "LGV_BRE2", "LGV_BRE0" })
        station_guingamp->linked_lines->push_back(line);

    auto station_saint_brieuc = StationDTO::createShared();
    station_saint_brieuc->label = "Saint-Brieuc";
    station_saint_brieuc->linked_lines = oatpp::List<oatpp::String>::createShared();
    for (auto line : { "LER_BRE0", "LGV_BRE2", "LGV_BRE0" })
        station_saint_brieuc->linked_lines->push_back(line);

    auto station_rennes = StationDTO::createShared();
    station_rennes->label = "Rennes";
    station_rennes->linked_lines = oatpp::List<oatpp::String>::createShared();
    for (auto line : {"LER_BRE0", "LER_NOR3", "LER_BRE2", "LER_BRE3","LGV_BRE0", "LER_BRE4", "LGV_FR4"})
        station_rennes->linked_lines->emplace_back(line);


    auto station_chartres = StationDTO::createShared();
    station_chartres->label = "Chartres";
    station_chartres->linked_lines = oatpp::List<oatpp::String>::createShared();
    for (auto line : {"LER_CVL1", "LGV_BRE0", "LGV_BRE1", "LGV_FR7", "LGV_FR9", "LGV_FR8"})
        station_chartres->linked_lines->push_back(line);


    auto station_massy = StationDTO::createShared();
    station_massy->label = "Massy TGV";
    station_massy->linked_lines = oatpp::List<oatpp::String>::createShared();
    for (auto line : {"LGV_BRE0", "LGV_BRE1", "LGV_FR7", "LGV_FR9", "LGV_FR8"})
        station_massy->linked_lines->push_back(line);
...
static void deserializeLineTest()
{
	...
	OATPP_ASSERT(tmp_guingamp->linked_lines->size() == 3);
	for (const auto& line_name : *(tmp_guingamp->linked_lines))
	...
	OATPP_ASSERT(tmp_saint_brieuc->linked_lines->size() == 3);
	for (const auto& line_name : *(tmp_saint_brieuc->linked_lines))
	...
	OATPP_ASSERT(tmp_rennes->linked_lines->size() == 7);
	for (const auto& line_name : *(tmp_rennes->linked_lines))
	...
	OATPP_ASSERT(tmp_chartres->linked_lines->size() == 6);
	for (const auto& line_name : *(tmp_chartres->linked_lines))
	...
	OATPP_ASSERT(tmp_massy->linked_lines->size() == 5);
	for (const auto& line_name : *(tmp_massy->linked_lines))
